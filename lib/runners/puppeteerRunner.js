...
async function puppeteerRunner (conf) {
  ...
  function isLinkExcluded(link) {
    index = exclusionList.length - 1;
    while (index >= 0) {
      if (link.includes(exclusionList[index])) {
        return true
      }
      index -= 1;
    }
  ...
  function dedupOutlinks(outlinks, outlinksCache) {
    ...
    while (index >= 0) {
      if (outlinksCache.includes(outlinksArray[index]) || outlinksArray[index].includes('#') || isLinkExcluded(outlinksArray[index])) {
